...
class YOLO:
    ...
    def train(self, epochvalx,train_stop,train_name,app,db, **kwargs):
        """
        ...
        print(kwargs)
        self.task = overrides.get('task') or self.task
        self.trainer = TASK_MAP[self.task][1](overrides=overrides, _callbacks=self.callbacks,epochval=epochvalx,train_stop=train_stop,train_name=train_name,app=app,db=db)
        if not overrides.get('resume'):  # manually set model only if not resuming
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model
        self.trainer.hub_session = self.session  # attach optional HUB session
        # print("@@@@@@@@@@@@",self.trainer.__stop__)
        self.trainer.train()
