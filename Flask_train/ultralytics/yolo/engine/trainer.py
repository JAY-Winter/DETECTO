...
# sys.path.insert(0, "C:/Users/sung/Desktop/ziptest/repository/repository.py")
...
class BaseTrainer:
    ...
    """
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None, epochval=None, train_stop=None, train_name=None,app=None, db=None):
        """
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            with self.epochval.get_lock():
                self.epochval.value = epoch + 1
            # f = open("C:/Users/sung/Desktop/ziptest/uploads/output2.txt",'r')
            # val = int(f.readline())
            # f.close()
            # if val == 1:
            #     f = open("C:/Users/sung/Desktop/ziptest/uploads/output2.txt",'w')
            #     f.write(str(0))
            #     f.close()
            #     self.stop = True
            #     break
            # f = open("C:/Users/sung/Desktop/ziptest/uploads/output.txt",'w')
            # f.write(str(epoch))
            # f.close()
            self.epoch = epoch
            ...
            with self.train_stop.get_lock():
                if self.train_stop.value:
                    self.stop = True
                    self.train_stop.value = False
                    break
            # Early Stopping
        ...
        with self.app.app_context():
            equipment = Equipment.query.get(self.train_name.value.decode())
            if not equipment:
                return
