...
def detect_non_wearing(pro, lists):
    global sum_dict
    print('################################################')
    print(f'[%] input lists: {lists}')  # [tensor([7., 2.], device='cuda:0')]
    ...
    for detected_list in lists:       # 이미지 한 장씩
        ...
        if len(detected_list) <= 0: 
            print('[%] 빈 배열')
            continue
        print(detected_list) # tensor([7., 2.], device='cuda:0')
        for i in range(len(detected_list)):  # tensor 안의 값들에 접근
            detected_class = int(detected_list[i].item())
            print(detected_class)
            if detected_class in pro:  # 이미지의 감지 클래스 하나씩
                no_wear.add(detected_class)
                print('[%] 추가 중: ', no_wear)
    print('[%] 이미지 4장 set: ', no_wear)
